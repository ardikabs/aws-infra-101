...
output "service_discovery_namespaces" {
  value = {
    for ns in toset(var.service_discovery_domains) : ns => {
      id  = aws_service_discovery_private_dns_namespace.default[ns].id
      arn = aws_service_discovery_private_dns_namespace.default[ns].arn
    }
  }
}
